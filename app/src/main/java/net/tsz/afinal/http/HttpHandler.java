...
public class HttpHandler<T> extends AsyncTask<Object, Object, Object> implements EntityCallBack {
    ...
    private final static int UPDATE_SUCCESS_ORIGINSTRING = 5;
    private final static int UPDATE_SUCCESS_SOAP_CONTENT_STRING = 6;
    ...
    protected void onProgressUpdate(Object... values) {
        ...
        switch (update) {
            ...
            case UPDATE_SUCCESS:
                ...
                break;
            case UPDATE_SUCCESS_ORIGINSTRING:
//				if(callback!=null)
//					callback.onSuccess_Origin_String(String.valueOf(values[1]));
                break;
            case UPDATE_SUCCESS_SOAP_CONTENT_STRING:
                if (callback != null)
                    callback.onSuccess_SoapContent(String.valueOf(values[1]));
                break;
            default:
                break;
        }
        ...
        if (status.getStatusCode() >= 300) {
            ...
            try {
                ...
                if (entity != null) {
                    ...
                    if (targetUrl != null) {
                        Object responseBody = mFileEntityHandler.handleEntity(entity, this, targetUrl, isResume);
                        publishProgress(UPDATE_SUCCESS, responseBody);
                    } else {
                        byte[] data = mStrEntityHandler.handleEntity(entity, this, charset);
                        String originString = new String(data);
                        publishProgress(UPDATE_SUCCESS_ORIGINSTRING, originString);
                        String responseBody = new String(data, charset);
                        //responseBody = mStrEntityHandler.handleEntity(entity,this,charset);
                        publishProgress(UPDATE_SUCCESS, responseBody);
                        if (responseBody != null && (responseBody.contains("<soap:Body") || responseBody.contains("<soapenv:Body"))) {
                            String soapContent = soapParser(new ByteArrayInputStream(data));
                            publishProgress(UPDATE_SUCCESS_SOAP_CONTENT_STRING, soapContent);
                        }
                    }
                } else {
                    publishProgress(UPDATE_SUCCESS, null);
                }
            } catch (Exception e) {
    ...
    private static String soapParser(InputStream in)
