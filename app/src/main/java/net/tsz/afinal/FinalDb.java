...
//				}
			...
			if(values.size()>0){
				...
				TableInfo info= TableInfo.get(entity.getClass());
				String column = info.getId().getColumn();
				String id = info.getId().getFieldName();
...
//	}
	...
	public boolean updateBindId(Object entity) {
		checkTableExist(entity.getClass());
		List<KeyValue> entityKvList = SqlBuilder.getUpdateKeyValueList(entity);
		if (entityKvList != null && entityKvList.size() > 0) {
			TableInfo tf = TableInfo.get(entity.getClass());
			Id sqlid=tf.getId();
			String column=sqlid.getColumn();
			Object data=sqlid.getValue(entity);
			ContentValues cv = new ContentValues();
			insertContentValues(entityKvList, cv);
			Long id = (long) db.update(tf.getTableName(), cv, column+"=?", new String[]{data.toString()});
			if (id<=0)
				return false;
			return true;
		}
	...
	public <T> T findById(Object id, Class<T> clazz) {
		...
		if (sqlInfo != null) {
			...
			Cursor cursor = null;
			try {
				cursor=db.rawQuery(sqlInfo.getSql(),
	...
	public <T> List<T> findAllByWhere(Class<T> clazz, String strWhere,
		...
		String sqlExtra="";
		if (orderBy!=null) {
			sqlExtra+=" ORDER BY "+orderBy;
		}
		if (limit!=null) {
			sqlExtra+=limit;
		}
		return findAllBySql(clazz,
				SqlBuilder.getSelectSQLByWhere(clazz, strWhere) + sqlExtra);
	}
	/**
	 * 根据条件查找所有数据
	 *
	...
	public long count(Class<?> clazz,String wheres) {
		checkTableExist(clazz);
		long num = -1;
		if (wheres!=null&&wheres.contains("where")){
			wheres="";
		}
		Cursor cursor = null;
		try {
			TableInfo table = TableInfo.get(clazz);
			String sql = "SELECT COUNT(*) FROM "+table.getTableName() + " "+wheres;
			debugSql(sql);
	...
	private boolean tableIsExist(TableInfo table) {
		if (table.isCheckDatabese())
			return true;
		if(tableIsExist(table.getTableName())){
